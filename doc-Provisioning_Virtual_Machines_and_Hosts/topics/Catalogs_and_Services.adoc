...
=== Example: Creating a Sample Service

{product-title} provides the ability to define services that can include one or more virtual machines or instances and can be deployed across hybrid environments. Services can be made available to users through the self-service user interface (SUI) that allows users to order predefined IT services without IT operations getting involved.

==== Creating a Sample Service for Provisioning a Virtual Machine through Red Hat Virtualization (RHV)


The following four items are required to make a service available to users from the {product-title_short} self-service user interface:
...
Every provider in {product-title_short} comes with a sample provisioning dialog covering the options specific to that provider. To have a look at the sample provisioning dialog for Red Hat Virtualization, go to menu:Automation[Automate > Customization > Provisioning Dialogs > VM Provision] and select *Sample RedHat VM Provisioning Dialog*. This is a textual representation of the dialog you will get when you provision a virtual machine or instance.

.Create a Service Dialog

When users access a service, the majority of options available to them are preset and cannot be altered. A service dialog exposes some of those options to the user so that even if they are ordering a basic RHEL 7 machine, for example, they can at least choose the amount of memory, virtual CPUs, or other options available to the instance they order. In cases where certain fields must be unique, such as the name of virtual machines in Red Hat Virtualization, users must enter their own unique name for the virtual machine they choose or the operation will fail, so this field must be exposed.
...
* Inside the boxes, one or more *Elements*. The *Elements* contain methods, like check boxes, drop-down lists or text fields, to fill in the options on the provisioning dialog.
...
.. Add a new element for the service name. Elements are controls that accept input.
...
. Select *RHEV* from *Catalog Item Type* list.
...
. Click the *Request Info* tab to enter the request details:
...
As part of the above sample service, you created a provisioning dialog that defines the options that can be set on a virtual machine and a service dialog which allows us to expose certain options to be set by the user. For our example, the service name, virtual machine name, description, and characteristics are configurable. Then, you created a service catalog and finally a catalog item. The catalog item combines the service dialog with all of the options in the provisioning dialog. You can now provision a virtual machine through Red Hat Virtualization from the Service User Interface (SUI).
...
. Click *Order*. You have ordered a new provisioning request. You can follow the request by clicking the *Orders* tab and then clicking the order to see its progress and details.
