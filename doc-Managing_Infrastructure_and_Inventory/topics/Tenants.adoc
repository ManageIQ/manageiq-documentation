...
{product-title_short} creates cloud tenants to match existing OpenStack tenants for managing resources and controlling visibility of objects. OpenStack tenant mapping must be enabled for cloud tenants to be created. See xref:tenant-mapping[] for details.
