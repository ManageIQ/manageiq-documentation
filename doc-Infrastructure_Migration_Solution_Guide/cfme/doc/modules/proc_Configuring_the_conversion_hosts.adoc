...
[IMPORTANT]
====
If you are using SSH transformation, you must also copy the VMware keys to the conversion hosts. The VMware keys are not used automatically, in order to prevent man-in-the-middle attacks. See xref:Ssh_only_copying_the_vmware_hypervisor_keys_to_the_conversion_hosts[].
====
+
...
If you are using SSH transformation, you can copy the VMware keys to the conversion hosts. See xref:Ssh_only_copying_the_vmware_hypervisor_keys_to_the_conversion_hosts[].
