[[expression-methods-overview]]
=== Expression Methods 

{product-title_short} additionally provides support for Expression Methods, that allow you to use advance search filters as Automate Methods, substituting the user input from Automate Objects. Expression methods have several distinct advantages, including: running directly in the worker appliance; removing the overhead of forking a DRb process to run the Automate Methods; no Ruby code required; and prebuilt for Dynamic Dialogs. 
...
|*arg* |The argument used in the expression. Each argument should employ the prefix *arg*. Example:
...
include::expression_method.adoc[]
