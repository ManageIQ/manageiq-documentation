...
====== Subscription Management for Virtual Environment

Customers may license CloudForms for a limited set of providers. This ability is enabled by providing entitlement certificates which describe the features to be enabled. CloudForms can be shipped as a bundled product with other Red Hat products like OpenStack and OpenShift, providing advanced management capabilities to these products.

For more information on managing subscriptions for an IT infrastructure, see, Red Hat Subscription Management https://access.redhat.com/documentation/en-US/Red_Hat_Subscription_Management/1/html/Subscription_Concepts_and_Workflows/index.html[Subscription Concepts and Workflows] guide.
