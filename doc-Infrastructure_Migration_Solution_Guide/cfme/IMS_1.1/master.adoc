...
== Preparing the environments for migration
...
This configuration involves adding the following steps to xref:Preparing_the_environment_for_migration_to_{context}[].

[cols="1,1", options="header"]
|===
|Section |Additional configuration for SSH transformation
|xref:Preparing_the_vmware_environment_{context}[] |xref:Configuring_the_vmware_hypervisors_for_ssh_transformation_{context}[]

You can collect the SSH keys from the hypervisors at this time, if your security policies allow.

The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_the_conversion_hosts_for_ssh_transformation_{context}[].
|xref:Installing_rhv-{context}[] |If the conversion hosts are using SSSD with single sign-on:  xref:Reinstalling_ipa_client[]
|xref:Configuring_conversion_hosts_for_vddk_with_Ansible-{context}[] |xref:Configuring_the_conversion_hosts_for_ssh_transformation_{context}[].
|After xref:Configuring_conversion_hosts_for_vddk_with_Ansible-{context}[] |xref:Copying_vmware_ssh_keys_to_conversion_hosts-{context}[]
|After xref:Configuring_conversion_hosts_for_vddk_with_Ansible-{context}[] |xref:Configuring_secure_remote_login_to_the_vmware_hypervisors[]
...
include::modules/proc_configuring_conversion_hosts_ssh_Ansible.adoc[leveloffset=+2]
...
You must install and configure Red Hat OpenStack Platform and CloudForms and deploy the conversion hosts for VDDK transformation.
...
You must create the Red Hat OpenStack Platform conversion hosts, download the VMware Virtual Disk Development Kit (VDDK), and configure and verify the Red Hat Virtualization conversion hosts with Ansible playbooks.
...
|===
|Section |Additional configuration for SSH transformation
|xref:Preparing_the_vmware_environment_{context}[] |xref:Configuring_the_vmware_hypervisors_for_ssh_transformation_{context}[Configure the VMware hypervisors for SSH transformation].

You can collect the SSH keys from the hypervisors at this time, if your security policies allow.

The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_the_conversion_hosts_for_ssh_transformation_{context}[].
|xref:Configuring_conversion_hosts_for_vddk_with_Ansible-{context}[] |xref:Configuring_the_conversion_hosts_for_ssh_transformation_{context}[Add the required parameters and values].
|After xref:Configuring_conversion_hosts_for_vddk_with_Ansible-{context}[] |xref:Copying_vmware_ssh_keys_to_conversion_hosts-{context}[Copy the VMware SSH keys to the conversion hosts]
|===
