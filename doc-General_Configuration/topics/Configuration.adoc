...
... Enter the *Repository Name(s)*. The default is `cf-me-5.6-for-rhel-7-rpms rhel-server-rhscl-7-rpms`, which are the {product-title} repository and the Red Hat Software Collections repository.
...
... Enter the *Repository Name(s)*. The default is `cf-me-5.6-for-rhel-7-rpms rhel-server-rhscl-7-rpms`, which are the Red Hat CloudForms repository and the Red Hat Software Collections repository.
... To use a HTTP proxy, select *Use HTTP Proxy* and enter your proxy details.
...
------
+
. In CloudForms 4.1, the following change is necessary to the `idp-metadata.xml` file for SAML logout to work between `mod_auth_mellon` and Red Hat SSO:
+
------
# vi idp-metadata.xml
...
[NOTE]
====
The use of rubyrep replication has been deprecated in this release of {product-title}; therefore, the database_synchronization role and replication worker are no longer used when using pglogical replication. 
====
