...
. Log into the {product-title} appliance as root using SSH, then create the `/etc/httpd/saml2/` directory:
+
------
...
------
+
The `mellon_create_metadata.sh` script creates file names based on the appliance URL, which must be renamed to match the expected file names from the `manageiq-external-auth-saml.conf` file:
+
------
...
+
. Then update the client definition (in SSO?) for the appliance with the following:
+
[options="header"]
|=========================================================================================
| Setting                                     | Value
...
|=========================================================================================
+
Then obtain the identity provider’s `idp-metadata.xml` metadata file as follows:
+
------
# cd /etc/httpd/saml2
...
------
+
See _SAML Assertions_ in xref:saml-assertions[] for more information. (Needed to perform this procedure?)

[[configure-appliance-UI-SAML]]
*Configure the Appliance Administrative User Interface*
...
[IMPORTANT]
======
Ensure the user’s groups are created on the appliance and appropriate roles are assigned to those groups.
======
+
. Click *Save*.
+
...
Alternatively, run the following command after using SSH to log into the appliance as root:
...
To authenticate to the appliance, the following remote user parameters are looked at by the appliance upon a successful login and redirect from the identity provider.
...
[[using-named-groups-to-assign-account-roles]]
==== Using Named Groups to Assign Account Roles
