...
** *Distinguished Name* (CN=<user>): Uses just the user name. Be sure to enter the correct *User Suffix*. Choose the correct *Distinguished Name* option for your directory service implementation: for example, `cn=dan bright,ou=users,dc=acme,dc=com` (The user logs on as `dan bright`.)
** *Distinguished Name* (UID=<user>): Uses just the UID. Be sure to enter the correct *User Suffix* for either one. Choose the correct *Distinguished Name* option for your directory service implementation: for example, `uid=dan bright,ou=users,dc=acme,dc=com`. (The user logs on as `dan bright`.)
...
In both LDAP and LDAPS, you can use groups from your directory service to set the role for the authenticated LDAP user. The LDAP user must be assigned one of the account role groups. See xref:ldap_groups[] for more information.
...
. Enter a description for the group in *Description*. (REMOVE?)
. There are two ways to specify the group to use:
* In the *Description* field, enter the canonical name (`cn`) for a valid LDAP group assigned to users requiring access to {product-title}.
* Select *Look Up LDAP Groups* to find a list of groups assigned to a specific user in LDAP, then use the *LDAP Group for User* dropdown list to choose a group.
...
In your directory service, define a distribution group for one or more of the account roles with the names shown in the table below.
...
[EDIT - NEED MORE DETAILS ON WHAT TO CHECK, ie. WHAT MESSAGES SHOW SUCCESS AND FAILURE.] You can also check your logs to verify your LDAP configuration is working correctly.
