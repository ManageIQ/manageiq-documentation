...
|
								...
								Orphaned: An orphaned virtual machine has no host but has a datastore associated with it. Orphaned virtual machines are those that have been removed from their providers but due to a connection error or process failure, still appear in the provider's storage during a CloudForms Management Engine SmartState analysis on the provider.
