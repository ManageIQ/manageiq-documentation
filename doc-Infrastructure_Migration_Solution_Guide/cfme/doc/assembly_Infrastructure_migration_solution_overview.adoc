...
+
* Preparing the source network and checking the virtual machines for anything that might hinder the migration process
* Configuring the target storage and networks, installing software, and adding the providers to CloudForms
* Configuring the conversion hosts (conversion hosts are RHV hosts or OpenStack Platform instances, running `virt-v2v` and `virt-v2v-wrapper`) in CloudForms
...
+
See xref:Vmware_to_rhv_migration_workflow[] or xref:Vmware_to_osp_migration_workflow[] for migration process details.
