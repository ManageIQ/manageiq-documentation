...
----
+
. Use the AWS-CLI tools to import a disk as a snapshot. See the link://docs.aws.amazon.com/AWS[AWS documentation]
on importing a disk as a snapshot using VM Import/Export.
+
----
$ aws ec2 import-snapshot --disk-containers file://containers.json --region BUCKET_REGION
----
+
...
----
. Create the AMI from the snapshot. See the link://docs.aws.amazon.com/AWS[AWS documentation] on creating a Linux AMI from a Snapshot.
+
----
