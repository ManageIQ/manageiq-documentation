...
You can assign a storage profile when provisioning a VMware virtual machine in {product-title}, by using the virtual machine as a template to clone. See https://access.redhat.com/documentation/en/red-hat-cloudforms/4.6/single/provisioning-virtual-machines-and-hosts/#provisioning-virtual-machines[Provisioning Virtual Machines] in _Provisioning Virtual Machines and Hosts_ for instructions.
