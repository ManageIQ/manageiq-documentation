...
[NOTE]
====
The procedure applies to Red Hat Virtualization 4.x versions. Red Hat Enterprise Virtualization 3.x versions are no longer supported 
====
...
. Enable PostgreSQL to listen for remote connections by updating the
  `listen_addresses` line in `/var/opt/rh/rh-postgresql95/lib/pgsql/data/postgresql.conf`:
+
------
listen_addresses  =  '*'
------
