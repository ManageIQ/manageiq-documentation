...
----
+
. Edit the `/etc/httpd/conf.d/manageiq-external-auth.conf` file to point to your Kerberos domain hosted by your Active Directory domain by adding or editing the lines for `KrbAuthRealms`, `Krb5KeyTab` and `KrbServiceName` for your environment:
+
----
