...
* During virtual machine provisioning, the Customize tab is hidden if the template has an unknown Operating System (OS) type. To make the Customize tab visible in the user interface, you will need to set the OS type from the provider or perform SmartState analysis on the template so the OS type is detected.
====
