...
The BIOS settings of the hosts should be set for optimal performance, rather than power-saving, according to the vendor's recommendations. Where applicable, the C1E halt state should be disabled.
...
See xref:proc_Creating_rhv_conversion_hosts[Creating RHV Conversion Hosts], xref:Configuring_rhv_conversion_hosts_in_CloudForms[Configuring RHV Conversion Hosts in CloudForms], and xref:Configuring_rhv_conversion_hosts_for_ssh_transformation[Configuring RHV Conversion Hosts for SSH Transformation] below.
