...
. (SSH only) Reinstalling `ipa-client`, if you are configuring the conversion hosts to use SSSD with single sign-on. See xref:Reinstalling_ipa_client[].
...
You can configure the conversion hosts.

If you are using SSH and configuring the conversion hosts to use SSSD with single sign-on, you can reinstall the `ipa-client`.
