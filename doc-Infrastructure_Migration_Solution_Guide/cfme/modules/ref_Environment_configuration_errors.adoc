...
+
The Red Hat OpenStack Platform `admin` user in CloudForms does not have `admin` role privileges in the target project. link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/16.0/html-single/users_and_identity_management_guide/#edit_a_project[Add the `admin` user as `member` and `admin`] to your target project.
+
