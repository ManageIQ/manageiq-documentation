...
. Click *OpenStack* from the list to download the appliance image.
endif::miq[]
...
The following procedure demonstrates creating a flavor with the minimum requirements (4 cores, 12 GB RAM, 44 GB disk space) for {product-title}. For more information about flavors, see the Red Hat OpenStack Platform Administration User Guide.
