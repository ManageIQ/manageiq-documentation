...
{product-title} also allows ISO provisioning from Red Hat Virtualization Manager datastores. 

*Requirements for ISO Provisioning*

* The *Kickstart* file in the root directory of the ISO must be named *ks.cfg*. This requires remastering of the ISO. 
* ISO provisioning requires a customization template to provide a Kickstart file for initial installation of the operating system. See xref:customization-templates-for-virtual-machine-and-instance-provisioning[] for more information on customization templates.
* ISO Provisioning on Red Hat Virtualization will write the content generated by the customization template to the file named `ks.cfg` that will be attached to the virtual machine using the RHV floppy payload. 
* For more information, see https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/installation_guide/sect-kickstart-howto[How Do You Perform a Kickstart Installation] in the Red Hat Enterprise Linux _Installation Guide_.


==== Initial Workflow
To use this feature, you will need to do the following before creating a provision request.

[NOTE]
====
* The Red Hat Virtualization Manager system must have already been discovered or added into the VMDB. For more information, see https://access.redhat.com/documentation/en-us/red_hat_cloudforms/4.7-Beta/html-single/managing_providers/#adding_a_red_hat_enterprise_virtualization_manager_provider[Adding a Red Hat Enterprise Virtualization Manager Provider] in _Managing Providers_. 
* For more information on uploading an ISO:
** Red Hat Virtualization 4.1 and below, see https://access.redhat.com/documentation/en-us/red_hat_virtualization/4.2/html-single/administration_guide/#sect-The_ISO_Uploader_Tool[ISO Uploader Tool]; 
** Red Hat Virtualization 4.2, see https://access.redhat.com/documentation/en-us/red_hat_virtualization/4.2/html-single/administration_guide/#Uploading_Images_to_a_Data_Storage_Domain_storage_tasks[Uploading Images to a Data Storage Domain]. 
====

. Add the *ISO Datastore*. 
. Refresh the *ISO Datastore*.
. Create a *System Image Type*.
. Set the *ISO Image Type*.
. *Create* a customization template that includes the Kickstart file.
...
[NOTE]
====
ISO provisioning is performed using a Kickstart file. The customization template is required to provide the Kickstart file for installation of the operating system.
====
...
* The *Kickstart* file in the root directory of the ISO must be named *ks.cfg*. This requires remastering of the ISO. 
* ISO Provisioning on Red Hat Virtualization will write the content generated by the customization template to the file named *ks.cfg* that will be attached to the virtual machine using the RHV floppy payload.  
* For more information, see link: https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/installation_guide/sect-kickstart-howto[How Do You Perform a Kickstart Installation] in the Red Hat Enterprise Linux _Installation Guide_.
* Set the new virtual machine to power down after provisioning is complete. Ensure that *Power on after creation* is unchecked on the Schedule tab when creating the VM provisioning dialog. 

===== Sample ISO Including Kickstart File

[NOTE]
====
For more information on modifing an ISO file, see https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/anaconda_customization_guide/sect-iso-images[Working with ISO Images] In the Red Hat Enterprise Linux _Anaconda Customization Guide_. 
====
RHEL ISO with the following modifications:

* Add a Kickstart file named `ks.cfg` to the root of the ISO. See example below. 
* Add `ks=cdrom:/ks.cfg` to the `append` line under the volume label in the `isolinux/isolinux.cfg` file to use the ISO as the installation method.   
+
-----
label linux
  menu label ^Install Red Hat Enterprise Linux 7.5
  kernel vmlinuz
  append initrd=initrd.img inst.stage2=hd:LABEL=RHEL-7.5 Server.x86_64 quiet ks=cdrom:/ks.cfg
-----
