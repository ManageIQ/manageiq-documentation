...
** *Distinguished Name* (CN=<user>) or *Distinguished Name* (UID=<user>): Uses just the user name. Be sure to enter the correct *User Suffix* for either one. Choose the correct *Distinguished Name* option for your directory service implementation: for example, `cn=dan bright,ou=users,dc=acme,dc=com` or `uid=dan bright,ou=users,dc=acme,dc=com`. (In this case, the user would log on as `dan bright`.)
...
[NOTE]
======
To get details of your LDAP settings related to a specific user, run:
...
In both LDAP and LDAPS, you can use groups from your directory service to set the role for the authenticated LDAP user. The LDAP user must be in one of the account role groups. See xref:ldap_groups[] for more information.
* Check *Get User Groups from LDAP* to retrieve the user's group membership from LDAP. This is used for mapping a user's authorization to a {product-title} role. This requires names to match groups on the
LDAP server side.
+
[IMPORTANT]
======
...
When leveraging your LDAP groups, if the LDAP user is not a member of any of the defined LDAP groups, then the user will be denied access to {product-title}.
...
. Enter a description for the group in *Description*.
...
* Select *Look Up LDAP Groups* to find a list of groups, then use the dropdown list in the *LDAP Group Information* area to choose a group.
...
To test a problematic CloudForms and Active Directory LDAP configuration, you can run the following to see if the user been pulled from Active Directory with the right group. For example:
