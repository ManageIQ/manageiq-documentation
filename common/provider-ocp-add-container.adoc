...
[NOTE]
====
The Image Inspector extracts OpenShift container images from the registry and obtains the Common Vulnerabilities and Exposures (CVE) information (for effective scanning) via a proxy.
====
+
...
.. Enter the *Image-Inspector Tag* value. The default is `2.1`. Tag is a mark used to differentiate images in a repository, typically by the application version stored in the image.
.. Enter the *CVE location*.
. Click *Add*.

[NOTE]
====
When the Image Inspector settings are not set for an individual OpenShift provider via the user interface, you can set the global default via the advanced settings menu by editing the values under `ems_kubernetes`.
