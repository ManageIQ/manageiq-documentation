...
* Set the new virtual machine to power down after provisioning is complete. Ensure the *Power on after creation* is unchecked on the Schedule tab when creating the VM provisioning dialog. 

===== Sample ISO Including Kickstart File

RHEL ISO with the following modifications:

* Add a kickstart file named `ks.cfg` to the root of the ISO using the below example. 
* Modify the isolinux/isolinux.cfg file to point to`ks.cfg`  
* Add the following to the append lines: ks=cdrom:/ks.cfg
