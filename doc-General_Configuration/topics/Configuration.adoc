...
Database replication and centralized administration are now enabled on your instances. You can navigate to menu:Compute[Infrastructure > Virtual Machines], then click *VMs & Templates* and select a virtual machine in the global region. In the *Multi Region* table, click *Connect to VM in its Region* to perform required operations in the remote region. 
