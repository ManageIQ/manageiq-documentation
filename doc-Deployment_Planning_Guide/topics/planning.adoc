...
. Configure `appliance2` to point to `appliance1` by specifying the IP address for `appliance1` in the `memcache_server: option` line. Configure this in the server's advanced settings by navigating to the settings menu, selecting *Configuration*, then the *Advanced* tab:
+
----
:session:
