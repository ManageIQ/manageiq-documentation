...
** If you choose *Database*, see xref:creating_a_user[] to create users. 
** If you choose *LDAP* or *LDAPS*, see xref:ldap_settings[] for more information on configuration. 
** If you choose *Amazon*, see xref:amazon_settings[].
...
* Use *LDAP Host Names* to specify the fully qualified domain names of your LDAP servers. {product-title} will search each host name in order until it finds one that authenticates the user.
...
** *User Principal Name*: Type the user name in the format of user@domainname. 
** *Email Address*: Logs in with the user's email address.
** *Distinguished Name* (CN=<user>) or *Distinguished Name* (UID=<user>): Uses just the user name. Be sure to enter the proper *User Suffix* for either one. Choose the correct *Distinguished Name* option for your directory service implementation. 
** *SAM Account Name*:
...
If you choose LDAP or LDAPS, you can use groups from your directory service to set the role for the authenticated LDAP user. The LDAP user must be in one of the account role groups. See xref:ldap_groups[] for more information.
* Check *Get User Groups from LDAP* to retrieve the user's group membership from LDAP. This will be used for mapping a user's authorization to a {product-title} role. 
+
[IMPORTANT]
======
...
======
* Check *Get Roles from Home Forest* to use the LDAP roles from the LDAP user's home forest.
...
* Specify the user name to bind to the LDAP server in *Bind DN*. This user must have read access to all users and groups that will be used for {product-title} authentication and role assignment. This example uses a service account user, `svc-ldap`, with access to all LDAP users.
* Enter the password for the Bind DN user in *Bind Password*.
. Click *Validate* to verify your settings.

LDAP authentication has now been configured in your CloudForms environment.

To assign account roles using LDAP groups, see xref:ldap_groups[].


====== Trusted Forests

If a user has group memberships in another LDAP Forest, specify the settings to access the memberships in the trusted forest.
...
// Break this procedure here, or contain in the same section?

[[ldap_groups]]
====== Assigning Account Roles Using LDAP Groups

After configuring LDAP authentication as described in xref:ldap_settings[], you can assign account roles. You can either create groups that match your pre-existing LDAP groups by name, or add the default groups from {product-title} to the LDAP server you are connecting to.

When leveraging your LDAP groups, if you are using LDAP and the LDAP user is not a member of any of the defined LDAP groups, then the user will be denied access to {product-title}.

There are two ways to use LDAP groups with {producttitle}:

* Use pre-existing groups from your LDAP server to assign {product-title} account roles.
...
* Type in the *cn* for the group in *LDAP Group*. This group must be in the LDAP directory source you specified under Operations-Server [QUESTION: Which field do you add this in? There is no longer an LDAP Group field].
* Select *Look Up LDAP Groups* to find a list of groups, then use the dropdown list that appears in the *LDAP Group Information* area to choose a group.
...
. Click the *Authentication* tab and enable *Get User Groups from LDAP* after typing in all of the required settings. See xref:ldap_settings[]. (QUESTION: What are the required settings?)

[[ldap_lookup_priority]]
====== Configuring Lookup Priority for LDAP Groups

{product-title} can have multiple LDAP groups configured, which the appliance will attempt to authenticate with one by one until it succeeds. However, the lookup priority of these groups can be arranged as desired. To configure the order in which {product-title} looks up LDAP groups:
...
. Change groups by clicking on the user dropdown menu on the top right of the screen. The dropdown list will show the groups the user is authorized for. 
