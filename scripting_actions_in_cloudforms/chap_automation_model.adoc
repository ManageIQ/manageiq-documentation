...
{product-title} Automate enables real-time, bi-directional
...
a task. {product-title} allows for Ruby methods. You can
...
set are subject to change, and have changed since {product-title} {product-version}
({product-title} 5.3). If you are calling gems using
Automate that are no longer in the {product-title}
...
For lists of Ruby gems included in different {product-title}
 releases, see the following article:
