...
. Navigate to menu:Configuration[Management > Providers].
...
Similarly, all discovered job templates are accessed under the provider by expanding the menu:Configuration[Management > Ansible Tower Job Templates] accordion menu.
...
. Navigate to menu:Configuration[Management > Configured Systems].
...
[NOTE]
====
Instead of running a single job at a time, multiple service catalog items can also be grouped together as a catalog bundle to create one deployment with multiple job templates. See https://access.redhat.com/documentation/en/red-hat-cloudforms/4.5/provisioning-virtual-machines-and-hosts/chapter-5-catalogs-and-services#catalogs-services[Catalogs and Services] in _Provisioning Virtual Machines and Hosts_ for more information.
====
