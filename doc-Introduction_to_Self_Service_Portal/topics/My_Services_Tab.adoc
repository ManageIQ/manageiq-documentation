...
[[vm-power-operations]]
=== Power Operating the Virtual Machine

Follow this procedure to perform power operations on virtual machines, that is start, stop, or suspend, depending on the current state of the virtual machine. 

. From the *Services* screen, click on the required service.
. Under *Resources*, click more options _(represented by three dots)_ where you have the option to start, stop, or suspend the virtual machine.
. Alternatively, click on the virtual machine name to see more details about the resource, including the *Power Operations* button. 
