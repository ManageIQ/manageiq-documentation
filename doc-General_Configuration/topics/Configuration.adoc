...
<<<<<<< HEAD
For more information on managing subscriptions for an IT infrastructure, see the Red Hat Subscription Management _Subscription Concepts and Workflows_.
...
<<<<<<< HEAD
For more information on migrating from older Red Hat Network Classic (hosted) to subscription management, see the Red Hat Subscription Management _Migrating from RHN Classic_.
=======
>>>>>>> darga
endif::cfme[]
...
.. Select *Active* to enable the backup schedule.
.. In the *Action* list, select *Database Backup*.
.. Select a type of server for storing the backups from the *Type* list. You can use *Network File System* (NFS) or *Samba*.
...
* *Hourly*: select the number of hours between backups from the list.
* *Daily*: select the number of days between backups from the list.
* *Weekly*: select the number of weeks between backups from the list.
* *Monthly*: select the number of months between backups from the list.
. Select a *Time Zone* for the schedule.
. Enter or select a *Starting Date* for the schedule.
