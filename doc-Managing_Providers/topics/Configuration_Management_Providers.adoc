...
Ansible Tower is a management tool designed to help automate infrastructure operations. Ansible Tower provides the ability to automate provisioning and configuration of infrastructure resources and applications in {product-title}, as requested through the self-service provisioning portal. This also includes deploying Red Hat Satellite agents on bare metal machines as required.

{product-title} allows you to execute Ansible Tower jobs using service catalogs and Automate. No custom configuration or Ruby scripting is needed in {product-title}, as configuration is done in Ansible Tower using playbooks. You can use the large library of existing Ansible playbooks as {product-title} state machines to automate tasks such as backups, package updates, and maintenance in your {product-title} environment. This can be particularly useful for quickly applying changes across large environments with many virtual machines or instances. Using Ansible Tower, you can schedule Ansible playbook runs and monitor current and historical results, allowing for troubleshooting or identification of issues before they occur.
...
. From the Ansible Tower job template, a new catalog item is created in {product-title}, optionally with a service dialog that allows the user to enter parameters if needed.
. The user orders the service from the {product-title} user interface, and if configured fills out any arguments (for example, limiting the task to run on a specific set of virtual machines). The job executes.
...
To add an Ansible Tower provider:
...
+
image:Add_a_new_Configuration_Management_Provider.png[Add_Ansible_Provider]
+
.. Type a *Name* for the new provider.
.. In the *Type* field, select *Ansible Tower* from the list.
.. Type the *URL* location or IP address to the Ansible Tower server.
. Click the *Verify Peer Certificate* checkbox if desired.
...
You can refresh inventory from {product-title}, or by enabling the *Update on Launch* option for inventory groups in Ansible Tower. The *Update on Launch* option allows Ansible to automatically update inventory using a dynamic inventory script before launching a Tower job from a playbook. See the link:http://docs.ansible.com/ansible-tower/index.html[Ansible Tower documentation] for more information.
...
+
image:refresh.png[Refresh_Ansible_Provider]
+
. Click *OK*.
...
. Click your Ansible Tower provider to expand and list the inventory groups on that Ansible Tower system. The inventory groups can be expanded to view the systems contained within each group, as well as configuration details for these systems.
...
.  Click  image:1847.png[Configuration] (*Configuration*), then  image:1862.png[Add a New Catalog Item] (*Add a New Catalog Item*) to create a new catalog item with at minimum the following details:
...
// (the service I ordered had an extra screen come up after -- Options > Limit, and Extra Variables: var_second, and *Submit*) . Document with the extra variables?
...
. Click  image:1847.png[] (*Configuration*), then click  image:1862.png[] (*Add a new Button*). 
  ...
  * Configure *Object Details* fields with the following request details:
    ...
    ** For *Request*, enter *Ansible_Tower_Job*. (QUESTION: Why name it this?)
