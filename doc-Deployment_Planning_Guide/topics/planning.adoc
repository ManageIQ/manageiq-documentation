...
//Ask SMEs Colin's questions from Comment 9, 10, 11: Any way we can turn off the memcached server running on these hosts? (unrelated resource contention issues suggest this would be useful)

However, running several appliances using a load balancer can cause login issues from the Self Service user interface because `memcached`, the service storing the session in memory, is shared (by appliances on the load balancer ?). To prevent this, configure your environment to run the `memcached` service on only one appliance with the steps below.
...
----
+
. On `appliance1`, configure the following in the server's advanced settings by navigating to the settings menu, selecting *Configuration*, then the *Advanced* tab:
+
----
:session:
...
----
+
This configures `memcached` to run on `appliance1`, but listen to all servers (-l 0.0.0.0). The `memcache_server: option` line, which specifies where it connects to, remains set as itself (127.0.0.1).
+
. Click *Save*.
. Restart `appliance1`:
+
...
. Configure `appliance2` to point to `appliance1`, using the IP address for `appliance1` in the below example. Configure the following in the server's advanced settings by navigating to the settings menu, selecting *Configuration*, then the *Advanced* tab:
+
----
:session:
...
[IMPORTANT]
====
When using this configuration, always start the appliance that owns `memcached` first, and ensure that `memcached` is running before starting the additional appliances. Starting another appliance before the `memcached` appliance may result in start or login issues.
====
