...
# URL of VDDK package. Location must be accessible over HTTP.
...
# List cloud or infrastructure providers.
# Each provider is a dictionary with attributes, `name` and `connection_configurations`.
manageiq_providers:
  - name: "_RHV_"
...
# You will copy this key to the hypervisors at a later stage if you configure the
# conversion hosts for SSH transformation.
v2v_ssh_private_key: |
...
[IMPORTANT]
====
If you run the `conversion_host_enable` playbook more than once on the same host, you will create multiple entries in the CloudForms database for the same host.

Before running the `conversion_host_enable` playbook again, remove the existing database entry by running the `conversion_host_disable` playbook on the host:
...
If you are configuring conversion hosts for SSH transformation, you must configure the SSH connection.

[id="SSH_only_Configuring_and_validating_the_SSH_connection"]
== SSH only: Configuring and validating the SSH connection

. Enable SSH on all VMware hypervisors. (For instructions, go to link:https://docs.vmware.com/en/VMware-vSphere/index.html[VMware vSphere Documentation]. In the left pane, click menu:vSphere _version_[ESXi and vCenter Server > VMware ESXi Installation and Setup > Installing and Setting Up ESXi > Setting Up ESXi > Enable ESXi Shell and SSH Access with the Direct Console User Interface].)

. On the Red Hat Virtualization Manager machine or on each OpenStack Platform conversion host, create an SSH key pair and copy the public key to each VMware hypervisor, to ensure that each conversion host has the SSH key of the VMware hypervisor in its `known_hosts` file:
+
[options="nowrap" subs="verbatim"]
----
