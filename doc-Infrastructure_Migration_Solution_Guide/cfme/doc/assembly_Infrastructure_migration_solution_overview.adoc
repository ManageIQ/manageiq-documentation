...
. Preparing the environment for migration (xref:Preparing_the_environment_for_migration[])
+
...
. Migrating the infrastructure in CloudForms (xref:Migrating_the_infrastructure[])
+
