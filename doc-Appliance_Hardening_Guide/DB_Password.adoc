[[_chap_red_hat_cloudforms_security_guide_setting_the_password_for_the_database_appliance]]
...
Restricting access to the database requires modifications to the [path]_/var/opt/rh/rh-postgresql94/lib/pgsql/data/pg_hba.conf_ file.
...
TYPE::
  This defines the access type, either local access from the database host ([literal]+local+), remote access from an external host regardless of encryption ([literal]+host+), external access with encryption ([literal]+hostssl+), or external access without encryption ([literal]+nohostssl+).

DATABASE::
  The name of the database the host can access.
  Use [literal]+all+ for all databases.

USER::
  The name of the user the host can use to access the database.
  Use [literal]+all+ for all users.
...
For example, you might change the default HBA rules to only allow remote access to the Red Hat CloudForms Management Engine database ([literal]+vmdb_production+) from hosts in a certain subnet.
