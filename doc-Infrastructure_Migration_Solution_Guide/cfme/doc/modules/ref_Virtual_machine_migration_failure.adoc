...
* SSH transformation failure: If you are using SSSD with single sign-on, SSH fails for the `vdsm` account. Reinstall `ipa-client` without configuring the OpenSSH client:[[SSH_transformation_fails]]
+
...
----
+
This issue cannot be resolved by changing the configuration file because the file is restored during upgrades.
