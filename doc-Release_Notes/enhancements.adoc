...
CloudForms Management Engine can now report a high watermark for the number of virtual machines on a provider over a one-month period, which can be used for billing. Reports can be queued to collect watermark information for host sockets per provider and for virtual machines per provider. These reports are located in the *Reports* menu under menu:Configuration Management[Providers].
