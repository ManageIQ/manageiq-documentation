...
[IMPORTANT]
====
After installing the {product-title} appliance, you must configure the database for {product-title}; see xref:configuring_a_database[Configuring a Database for {product-title}].
====
...
Although it is recommended to use the PowerShell method, you can upload the appliance to an Azure environment using the Azure Command-Line Interface (Azure CLI). See link:https://azure.microsoft.com/en-us/documentation/articles/xplat-cli-azure-resource-manager/[] for steps on how to create and manage Azure resources using the Azure CLI. 

Azure requires that the uploaded Virtual Hard Disk (VHD) files are in a fixed format. The {product-title_short} virtual appliance image .vhd file is dynamic by default. Currently, the Azure CLI does not automatically convert the dynamic .vhd file to fixed during upload. To upload using the Azure CLI, the {product-title_short} virtual appliance image .vhd file must be first converted from dynamic to fixed, and properly aligned. 

Therefore, the procedure to upload the {product-title} appliance using Azure CLI involves:

. xref:installing-azure-cli[Installing Azure CLI 2.0]
. xref:converting-aligning-cfme-appliance-image[Converting and Aligning the {product-title} Virtual Appliance Image]
. xref:uploading-converted-cfme-appliance-image[Uploading and Provisioning the Fixed-Size {product-title} Azure Virtual Appliance Image].
...
Complete the following procedure to ensure the {product-title} dynamic .vhd file is aligned correctly, and is in a fixed-size VHD format. All .vhd files provisioned for Microsoft Azure must be aligned to the nearest 1 MB boundary and be in a fixed-size VHD format.
...
[[uploading-converted-cfme-appliance-image]]
===== Uploading and Provisioning the Fixed-Size {product-title} Azure Virtual Appliance Image

Complete the following steps to upload and provision the virtual appliance image you converted to a fixed-size VHD format and properly aligned per requirement.


[NOTE]
====
The exported storage connection string does not persist after a system reboot. If any of the commands in the following steps fail, export the storage connection string again using the commands below.
...
You have successfully provisioned a Red Hat CloudForms virtual appliance in Microsoft Azure. 
