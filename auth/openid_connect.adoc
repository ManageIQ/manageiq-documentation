...
Optional parametes can be appended to the end fo the configuration file to avoid SSL verification. These can
...
When the `--oidc-insecure` is specified not SSL verification will be done when
attempting to fetch the `--oidc-introspection-endpoint`, if it is not specified,
and SSL verification will be turned off using these parameters as described above
`OIDCSSLValidateServer` and `OIDCOAuthSSLValidateServer`
