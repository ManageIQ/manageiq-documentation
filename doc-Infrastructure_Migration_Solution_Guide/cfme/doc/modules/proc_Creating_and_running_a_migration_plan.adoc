...
+
To cancel a migration plan that has started, see xref:Canceling_a_migration_plan[].
...
If you cannot discover or import the source virtual machines, see xref:Migration_plan_errors[].
...
* *Save migration plan to run later* (see xref:Running_migration_plan_immediately[])
* *Start migration immediately* (see xref:Running_migration_plan_immediately[])
