...
Red Hat Infrastructure Migration Solution (IMS) 1.3 enables you to migrate virtual machines from VMware{nbsp}6.0 (or later) to  Red Hat Virtualization or Red Hat OpenStack Platform, using Red Hat CloudForms:
...
+
See xref:Migrating_from_vmware_to_rhv_1-3_vddk[].

Red Hat OpenStack Platform 13 (or later)::
Red Hat OpenStack Platform (RHOSP) provides a scaleable, fault-tolerant, private or public cloud based on Red Hat Enterprise Linux. See link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform[RHOSP] documentation for more information.
+
See xref:Migrating_from_vmware_to_osp_1-3_vddk[].

Red Hat CloudForms 5.0::
Red Hat CloudForms is the environment in which you perform the migration. CloudForms is a unified set of management tools for use across virtualized, private cloud, and public cloud platforms. See link:https://access.redhat.com/documentation/en-us/red_hat_cloudforms/5.0[CloudForms 5.0] documentation for more information.
...
This guide describes how to plan your migration, to prepare the VMware and Red Hat Virtualization (RHV) environments, and to migrate your VMware virtual machines.
...
You can prepare the VMware and Red Hat Virtualization (RHV) environments for migration.

The virtual disks are converted with the VMware Virtual Disk Development Kit (VDDK). If you cannot use VDDK, xref:Configuring_environment_for_ssh_transformation_rhv_1-3_ssh[SSH transformation] is a fallback option.
...
You can download VDDK and then configure the Red Hat Virtualization (RHV) conversion hosts in the CloudForms user interface.
...
|After xref:Preparing_the_vmware_environment_rhv_1-3_vddk[Preparing the VMware environment] a|Go to xref:Configuring_the_vmware_hypervisors_for_ssh_transformation_rhv_1-3_ssh[Configuring the VMware hypervisors for SSH transformation]

You can collect the SSH keys from the hypervisors at this time, if your security policies allow.

The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_cloudforms_rhv_1-3_ssh[Configuring the RHV conversion hosts for SSH transformation].

|After xref:Installing_configuring_rhv_rhv_1-3_vddk[Installing and configuring Red Hat Virtualization] |If the conversion hosts are using SSSD with single sign-on, go to xref:Reinstalling_ipa_client_{context}[Reinstalling `ipa-client`]

|Instead of xref:Configuring_conversion_hosts_cloudforms_rhv_1-3_vddk[Configuring the RHV conversion hosts for VDDK transformation] <a|Go to:

* xref:Configuring_conversion_hosts_cloudforms_rhv_1-3_ssh[Configuring the RHV conversion hosts for SSH transformation]
* xref:Copying_vmware_ssh_keys_to_conversion_hosts_{context}[Copying the VMware SSH keys to the RHV conversion hosts]
|===
...
You can prepare the VMware source and Red Hat Virtualization target environments for migration.

The virtual disks are converted with the VMware Virtual Disk Development Kit (VDDK). If you cannot use VDDK, xref:Configuring_environment_for_ssh_transformation_osp_1-3_ssh[SSH transformation] is a fallback option.
...
You can install and configure Red Hat OpenStack Platform (RHOSP) and CloudForms for migration.
...
You can create the Red Hat OpenStack Platform (RHOSP) conversion hosts, downhload VDDK, and configure the conversion hosts in the CloudForms user interface.
...
|===
|Section |Additional configuration for SSH transformation
|After xref:Preparing_the_vmware_environment_osp_1-3_vddk[Preparing the VMware environment] a|Go to xref:Configuring_the_vmware_hypervisors_for_ssh_transformation_{context}[Configuring the VMware hypervisors for SSH transformation]

You can collect the SSH keys from the hypervisors at this time, if your security policies allow.

The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_cloudforms_osp_1-3_ssh[Configuring the RHOSP conversion hosts for SSH transformation].

|Instead of xref:Configuring_conversion_hosts_cloudforms_osp_1-3_vddk[Configuring the RHOSP conversion hosts for VDDK transformation] <a|Go to:

* xref:Configuring_conversion_hosts_cloudforms_osp_1-3_ssh[Configuring the RHOSP conversion hosts for SSH transformation]
* xref:Copying_vmware_ssh_keys_to_conversion_hosts_osp_1-3_ssh[Copying the VMware SSH keys to the RHOSP conversion hosts]
|===
