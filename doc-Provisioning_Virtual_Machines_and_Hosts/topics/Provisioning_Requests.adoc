...
* During virtual machine provisioning, the Customize tab is hidden if the template has an unknown Operating System (OS) type. In that case, to make Customize tab available in the user interface, you will need to set the OS type from the provider or perform SmartState analysis on the template so the OS type is detected.
====
