...
. Run the `ls` command to locate the directory named after a GUID, and change to that directory. For example:
+
----
[root@localhost rhvexports]# ls
4a8fc2b1-0a57-47fd-b622-7e170981305b    __DIRECT_IO_TEST__
cfme-rhevm-5.7.0.17-1.x86_64.rhevm.ova

[root@localhost rhvexports]# cd 4a8fc2b1-0a57-47fd-b622-7e170981305b/
----
+
...
----
+
. Set the following permissions on the `images` and `master` directories so the appliance can be imported as a template:
+
----
...
The Red Hat Virtualization export domain now shows the new appliance in the administration portal.
