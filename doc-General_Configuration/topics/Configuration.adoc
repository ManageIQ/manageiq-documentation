...
* From the *User Type* list, select one of the following: [NOTE: NEED DETAILS ON WHAT THESE USER TYPES DO AND HOW EXACTLY TO CONFIGURE. IS THIS FOR WHEN YOU LOG IN?]
...
** Select *Distinguished Name* (CN=<user>) or *Distinguished Name* (UID=<user>) to use just the user name, but be sure to enter the proper *User Suffix* for either one [NOTE: WHAT'S THE PROPER SUFFIX?]. Choose the correct *Distinguished Name* option for your directory service implementation.
** Select *SAM Account Name* to  NEEDINFO ????
...
* Check *Get User Groups from LDAP* to *[QUESTION: What does this set?]*. ADD IMPORTANT HERE
+
[NOTE]
======
...
After configuring LDAP authentication as described in xref:ldap_settings[], you can assign account roles. You can either create groups that match your pre-existing LDAP groups by name, or add {product-title} default groups to the LDAP (FOREST/NETWORK?) you are connecting to.

When leveraging your LDAP groups, if you are using LDAP and the LDAP user is not a member of any of the defined groups, then the user will be denied access to {product-title}. *CLARIFY*
...
* Type in the *cn* for the group in *LDAP Group*. This group must be in the LDAP directory source you specified under [LINK Operations-Server].
+
image:2090.png[]
+
...
. Click the *Authentication* tab and enable *Get User Groups from LDAP* after typing in all of the required settings. See xref:ldap_settings[]. (DOES THIS MAKE SENSE?)?????
