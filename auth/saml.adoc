...
the user to specify a copy by file path that was downloaded to the appliance or by URL
in which case the CLI downloads the copy from the IDP.
...
on Login In to Corporate System to get redirected to the SAML login page. With this option
...
With both --saml-config and --saml-unconfig subcommands, the httpd service will be restarted
