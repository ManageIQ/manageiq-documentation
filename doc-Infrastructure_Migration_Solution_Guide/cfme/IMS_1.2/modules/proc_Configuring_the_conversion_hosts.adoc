...
. Log in to each the conversion host using SSH.
...
. Download the latest conversion host appliance. See xref:Deploying_osp_conversion_hosts[].
. Re-deploy the conversion hosts.
====
...
. Select a Red Hat Virtualization *Cluster* / Red Hat OpenStack Platform *Project*.
...
[IMPORTANT]
====
*Red Hat Virtualization*: If a Red Hat Virtualization host has an existing SSH private key, you must delete the old key manually in `/var/lib/vdsm/.ssh/id_rsa` before configuring the conversion host. Conversion host configuration does not overwrite existing keys.
====
...
* xref:Copying_the_vmware_keys_for_SSH_for_osp[Red Hat OpenStack Platform]
