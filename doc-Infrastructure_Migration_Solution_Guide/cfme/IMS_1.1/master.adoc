...
|After xref:Preparing_the_vmware_environment_{context}[Preparing the VMware environment] |Go to xref:Configuring_the_vmware_hypervisors_for_ssh_transformation_{context}[Configuring the VMware hypervisors for SSH transformation]

You can collect the SSH keys from the hypervisors at this time, if your security policies allow.

The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_ansible_ssh_{context}[Configuring the conversion hosts for SSH transformation].
|After xref:Installing_configuring_rhv_{context}[Installing and configuring Red Hat Virtualization] |If the conversion hosts are using SSSD with single sign-on, go to xref:Reinstalling_ipa_client_{context}[Reinstalling `ipa-client`]
|During xref:Configuring_conversion_hosts_ansible_vddk_{context}[Configuring the conversion hosts for VDDK transformation] |Go to xref:Configuring_conversion_hosts_ansible_ssh_{context}[Configuring the conversion hosts for SSH transformation] for additional parameters
|After configuring the conversion hosts for SSH transformation <a|Go to:
...
* xref:Authenticating_conversion_hosts_in_cloudforms_{context}[Authenticating the Red Hat Virtualization conversion hosts in CloudForms]
|===
...
The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_ansible_ssh_{context}[Configuring the conversion hosts for SSH transformation].
|During xref:Configuring_conversion_hosts_ansible_vddk_{context}[Configuring the conversion hosts for VDDK transformation] |Go to xref:Configuring_conversion_hosts_ansible_ssh_{context}[Configuring the conversion hosts for SSH transformation] for additional parameters
|After configuring the conversion hosts for SSH transformation |Go to xref:Copying_vmware_ssh_keys_to_conversion_hosts_{context}[Copying the VMware SSH keys to the conversion hosts]
|===
