...
. Install Red Hat Virtualization Manager on the Manager machine (see the link:https://access.redhat.com/documentation/en-us/red_hat_virtualization/4.3/html-single/installation_guide/#part-Installing_the_Red_Hat_Virtualization_Manager[Part I. Installing the Red Hat Virtualization Manager] in the _Red Hat Virtualization Installation Guide_).
...
+
Some of these hosts will be deployed as conversion hosts. The number of conversion hosts depends on your migration size and infrastructure (xref:Deciding_how_many_conversion_hosts_to_create[]).

. Enable the following ports in the conversion host network (see https://access.redhat.com/articles/417343[] for details):
* 22 - SSH
...
[NOTE]
====
Do not remove or change a provider (including refreshing RHV hosts) before, during, or after migration because these actions cause errors in infrastructure mappings and migration plans.
====
