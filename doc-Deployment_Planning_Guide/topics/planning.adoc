...
Centralized administration capabilities in {product-title} currently includes support for the following life cycle management operations. These operations can be initiated from the master region, and processed and executed on the remote region.
...
With centralized administration, the remote_queue_put leverages a new system-to-system REST API request to forward the original request to the target region. This request will be put on the local queue in the target region; the request will then be delivered by a worker in the target region as if it was queued there all along.

image:central-admin.png[Centralized Administration diagram]

[NOTE]
====
The operations initiated from the master region are subject to the role-based access control (RBAC) rules on the master region, which also includes the role-based access control of all the remote regions from replication. 
====
