...
+
See xref:Migrating_from_vmware_to_rhv_1-2[].
...
+
See xref:Migrating_from_vmware_to_osp_1-2[].
...
include::modules/proc_Configuring_conversion_hosts_cloudforms_vddk.adoc[leveloffset=+3]
...
The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_cloudforms_ssh_{context}[].
|xref:Installing_rhv-{context}[] |If the conversion hosts are using SSSD with single sign-on:  xref:Reinstalling_ipa_client[]
|xref:Configuring_conversion_hosts_cloudforms_vddk_{context}[] |xref:Configuring_conversion_hosts_cloudforms_ssh_{context}[].
|After xref:Configuring_conversion_hosts_cloudforms_vddk_{context}[] |xref:Copying_vmware_ssh_keys_to_conversion_hosts-{context}[]
|===
...
include::modules/proc_Configuring_conversion_hosts_cloudforms_ssh.adoc[leveloffset=+2]
...
You must install and configure Red Hat OpenStack Platform and CloudForms and deploy the Red Hat OpenStack Platform conversion hosts for VDDK transformation.
...
Deploying the Red Hat OpenStack Platform conversion hosts involves the following tasks:

. Creating the conversion hosts from the conversion appliance
. Downloading the VMware Virtual Disk Development Kit (VDDK)
. Configuring the conversion hosts in the CloudForms user interface
...
The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_cloudforms_ssh_{context}[].
|xref:Configuring_conversion_hosts_cloudforms_vddk_{context}[] |xref:Configuring_conversion_hosts_cloudforms_ssh_{context}[Add the required parameters and values].
|After xref:Configuring_conversion_hosts_cloudforms_vddk_{context}[] |xref:Copying_vmware_ssh_keys_to_conversion_hosts-{context}[Copy the VMware SSH keys to the conversion hosts]
|===
