...
/etc/sssd/sssd.conf
With the above configuration files in place, HTTP will query, using dbus, SSSD which will perform the LDAP/S authentication and authorization steps necessary to validate users and groups.
