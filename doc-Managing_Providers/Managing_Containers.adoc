...
Red Hat CloudForms 4.0 allows for container management along with image and flavor management for instances. The orchestration system can be OpenShift Enterprise or Atomic Enterprise Platform, and it can work with both RHEV and VMware providers.
...
* *Replicator* ensures that there is always a certain number of pods available. 
...
* Additional information, such as services, events, projects, routes (OpenShift Enterprise only), and metrics.
