...
* *SSL without validation*: Authenticate to the provider insecurely using SSL.
* *SSL*: Authenticate to the provider securely using a public Certificate Authority. Select this option if the provider has a valid SSL certificate and it is signed by a public Certificate Authority. No further configuration is required for this option. This is the recommended authentication method.
