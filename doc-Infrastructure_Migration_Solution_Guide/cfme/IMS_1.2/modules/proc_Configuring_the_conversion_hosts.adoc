...
To prevent man-in-the-middle attacks, the conversion hosts do not accept the SSH public keys of the VMware hypervisors automatically. Instead, the `known_hosts` file of each conversion host is manually populated with the keys.

Copying the VMware public keys to the conversion hosts involves the following key tasks:

. Collecting the SSH public keys of the VMware hypervisors, using one of the following methods:

* You can collect the keys when you enable SSH access on the VMware hypervisors. See   xref:Configuring_the_vmware_hypervisors_for_ssh_transformation[].
* You can collect the keys with `ssh-keyscan`

. Copying the keys to the conversion hosts
. Verifying the SSH connection between each conversion host and each VMware hypervisor
