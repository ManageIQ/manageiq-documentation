...
To collect capacity and utilization data for Red Hat Virtualization, you must add {product-title} as
...
------
+
. Create a new user for {product-title_short} and grant read-only access to tables and views:
+
------
...
------
+
. Enable external md5 authentication by appending the following line to `/var/lib/pgsql/data/pg_hba.conf`, or optionally to
...
. Enable PostgreSQL to listen for remote connections by updating the
  `listen_addresses` line in `/var/lib/pgsql/data/postgresql.conf`, or optionally in `/var/opt/rh/rh-postgresql95/lib/pgsql/data/postgresql.conf`:
+
------
listen_addresses  =  '*'
------
