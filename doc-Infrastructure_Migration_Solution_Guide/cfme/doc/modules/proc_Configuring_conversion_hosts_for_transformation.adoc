...
. Copy the public key you created in xref:Configuring_conversion_hosts_for_transformation[] to each VMware hypervisor to ensure that the Manager or the OpenStack Platform conversion host has the SSH key of the VMware hypervisor in its `known_hosts` file:
+
[options="nowrap" subs="+quotes,verbatim"]
----
