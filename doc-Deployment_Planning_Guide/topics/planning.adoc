...
|`postgresql.conf` |Data Directory(exact location?)|Default server configuration|Adds directive to include `/etc/manageiq/postgresql.conf.d` directory
|`postgresql.auto.conf` |Data Directory |Contains configuration set by the `ALTER SYSTEM` command|*Do not edit manually*
|`01_miq_overrides.conf` |`/etc/manageiq/postgresql.conf.d`|Contains miq/cfme default overrides |Overwritten on upgrades
|<other files>|`/etc/manageiq/postgresql.conf.d`|Contains user overrides |Takes precedence if alphabetical after `01_miq_overrides.conf`
|===
