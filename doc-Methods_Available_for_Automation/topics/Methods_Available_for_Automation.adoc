...
ifdef::cfme[]
The link:https://access.redhat.com/documentation/en/red-hat-cloudforms/4.6/release-notes/release-notes[Red Hat CloudForms 4.6 Release Notes], list Ruby gems that have been added, updated, or removed in the latest version of CloudForms Management Engine.
...
|
							add_disk footnoteref:[1,Supported for Red Hat Virtualization and VMWare virtual machines]
							
|

							Add storage to an existing VM. This method accepts as parameters: disk_name, disk_size, and options to pass values for `:thinProvisioned`, `:dependent`, and `:persistent`. Example: `vm.add_disk("[#{vm.storage_name}]", size_in_mb, thin_provisioned: true)`
