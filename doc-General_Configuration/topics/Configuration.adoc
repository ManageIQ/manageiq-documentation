...
Preserve data by performing a full binary backup from a remote server, and restore it to the same server. 
This includes all databases, users and roles, replication slots and other objects. 
The result of this is a full replacement of the PG data directory on the target database server using the `pg_basebackup` utility.

==== Performing a Binary Backup

Create a backup from the remote server and store it as a tar file inside the appliance-backup directory. 

. SSH into the database server as the *root* user.
+
. Enter the `pg_basebackup` command.
-----
# pg_basebackup -x -h <ADDRESS> -U root -Ft -z -D miq-backup<WHAT IS CFME directory?>
-----
...
+
-----
# scp miq-backup/base.tar.gz root@<ip@ of DB server>:/var/www/cfme
-----
+
. SSH to the target VM and stop both the EVM and PostgreSQL servers.
+
-----
