...
{product-title} supports the provisioning of VMware ESX hypervisors. To provision a virtual machine from VMware providers, you must have an appliance with the Automation Engine role enabled.
...
There are four types of provisioning requests available in {product-title}:
...
. Navigate to menu:Compute[Infrastructure > Virtual Machines], and select the virtual machine you want to clone.
...
. Navigate to menu:Compute[Infrastructure > Virtual Machines], and select the virtual machine you want to publish as a template.
. Click image:2007.png[](*Lifecycle*), and then image:2340.png[](*Publish selected VM to a Template*).
. Enter the requested information in the dialogs. Be sure to check the *Catalog* tab.
. Schedule the request on the *Schedule* tab.
. Click *Submit*.

[[renaming-a-provisioned-virtual-machine]]
==== Renaming a Provisioned Virtual Machine (VMware Virtual Machines Only)

{product-title} allows you to rename a VMware virtual machine without having to reprovision it. 

To rename a VMware virtual machine:

. Navigate to menu:Compute[Infrastructure > Virtual Machines], and select the VMware virtual machine you want to rename.
. Click image:1847.png[](*Configuration*), image:1851.png[](*Rename selected item*).
...
[[provisioning-a-virtual-machine-using-cloud-init-REST-API]]
==== Provisioning a Virtual Machine using cloud-init via REST API

To provision a virtual machine using cloud-init, the template that the virtual machine is provisioned from should have the `cloud-init` package installed. Also, the template should have the `use cloud-init/sysprep` checkbox selected.

See example below of a virtual machine provision request using cloud-init.
...
For a virtual provision request via REST API, if the below two fields in the request’s body are not set correctly, the cloud-init may not work:
...
////
. <empty>  (should be written with the <..>) No vNIC profile
. <Template> (should be written with the <..>) Use template's default vNIC profile
. profile_name (network_name) Set the specified vnic profile. Note that it is important to put one space between profile_name (network_name), otherwise it will fail. For example, ovirtmgmt (ovirtmgmt).  
. vnic profile id Set the specified vnic profile, such as "3a8dce01-dd59-4a46-8a6f-823acccda79f" 
////

[width="100%",cols="20%,40%,40%",options="header",]
|====
|vlan value|Description|vNIC profile
|<empty>|should be written with the <..>|No vNIC profile
|<Template>|should be written with the <..>|Use template's default vNIC profile
|profile_name (network_name)|It is important to put one space between profile_name (network_name), otherwise it will fail. For example, *ovirtmgmt (ovirtmgmt)*|Set the specified vNIC profile.
|vNIC profile ID||Set the specified vNIC profile, such as `3a8dce01-dd59-4a46-8a6f-823acccda79f`.
|====
...
Here are some possible cases, where VLAN field is not picked correctly, and this may lead to this validation failure:  
"Provision failed for the following reasons:\n'Network/Virtual NIC Profile ID or 
 Profile Name (Network Name)' is required"
...
Non-exist network name
Wrong format (missing space before the first bracket): “profile_name(network_name)” 
Use Network name (that was used in V3) instead of:  “profile_name(network_name)” 


Examples for wrong usage:
...
2. "sysprep_enabled"
Here are all possible values for this. 
It is written by this order:
Value - It’s meaning - how it appears in CFME UI VM provision dialog, “Customize” tab, “Customize” section.


For windows source template:
"sysprep_enabled": "fields" - Sysprep Specification - “Sysprep Specification”
"sysprep_enabled": “file” - Sysprep answer file - “Sysprep answer file”


For Linux template:
"sysprep_enabled": “fields” - Customized template and any Customized parameters will be used - “Specification”

To have cloud-init work (have the provisioned VM marked with “Use cloud-init”, and that the customized template will be used, as well as Customized parameters, if any) it is a must that  "sysprep_enabled" will be set to “fields”.
Otherwise, the customized template will be ignored, and the provision VM will not be marked with “use cloud-init” even though the template from which the VM is provisioned, has it marked.
And common for both:
"sysprep_enabled": “disabled” -  Do not customize - “<None>”
The customized template will be ignored, and the provision VM will not be marked with “use cloud-init” even though the template from which the VM is provisioned, has it marked.
