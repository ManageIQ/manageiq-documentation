...
* See https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Virtualization/3.6/html-single/Virtual_Machine_Management_Guide/index.html#sect-Using_Cloud-Init_to_Automate_the_Configuration_of_Virtual_Machines[Using Cloud-Init to Automate the Configuration of Virtual Machines] in the Red Hat Virtualization _Administration Guide_ for more information on using Cloud-Init in a Red Hat Enterprise Virtualization environment.
...
include::Provisioning_Requests-openstack.adoc[]

==== Provisioning a Google Compute Engine Instance from an Image

From Red Hat {product-title} 4.1, a Google Compute Engine instance can be provisioned. Some functionality available in other cloud providers is currently unavailable when provisioning from Google Compute Engine.
...
* Provisioning requires the *Automation Engine* server role be enabled. Confirm your server role settings in menu:Settings[Configuration > Server > Server Control].
...
[NOTE]
====
The default dialogs show all possible parameters for provisioning. To limit the options shown, xref:provisioning-dialogs-customizing[].
====
...
include::Provisioning_Requests-keypairs.adoc[]
