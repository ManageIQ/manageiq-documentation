...
* Metrics Count: {product-title_short} will create a number of records per minute, rolled up hourly and daily, for each provider instance once capacity and utilization is enabled. See the second table for information on VMDB sizing to include metrics gathering.



Use the following table as a guideline to calculate minimum requirements for your database without capacity and utilization enabled:
...
Host:: A computer running a hypervisor, capable of hosting and monitoring virtual machines. Supported hypervisors include RHV-H, VMware ESX hosts, Windows Hyper-V hosts.
