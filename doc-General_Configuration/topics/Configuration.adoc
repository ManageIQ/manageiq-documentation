...
This procedure uses the `pg_basebackup` utility to create a full replacement of the PostgreSQL data directory, capturing the exact state of the database when the backup finishes.

===== Performing a Binary Backup

Create a binary backup and store it as a tar file inside the {product-title_short} backup directory. 

[NOTE]
======
PostgreSQL superuser with _Replication_ permissions are required to perform this procedure. 
======
