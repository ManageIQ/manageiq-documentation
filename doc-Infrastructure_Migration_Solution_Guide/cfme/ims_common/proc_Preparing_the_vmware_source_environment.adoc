...
. Prepare the source virtual machines (xref:Preparing_the_source_virtual_machines[]).
. If you are using SSH transformation, configure the VMware hypervisors (xref:Configuring_the_vmware_hypervisors_for_ssh_transformation[]).
