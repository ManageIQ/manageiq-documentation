...
NOTE: For the metrics collection to work properly, you also need to configure the CloudForms Management Engine to allow for all three +Capacity & Utilization+ server roles which are available under menu:Configure[Configuration > Server > Server Control]. For more information on capacity and utilization collection, see the _Deployment Planning Guide_ available from https://access.redhat.com/documentation/en/red-hat-cloudforms/.
