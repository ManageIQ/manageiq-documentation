...
Red Hat OpenStack Platform (RHOSP) provides a scaleable, fault-tolerant, private or public cloud based on Red Hat Enterprise Linux (RHEL). See link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform[RHOSP documentation] for more information.
+
...
You can prepare the VMware source and RHV target environments for migration.
...
You can download VDDK, configure the Red Hat Virtualization (RHV) conversion hosts with Ansible playbooks, and authenticate the conversion hosts in CloudForms.
...
The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_ansible_rhv_1-1_ssh[Configuring the RHV conversion hosts for SSH transformation].
...
|Instead of xref:Configuring_conversion_hosts_ansible_rhv_1-1_vddk[Configuring the RHV conversion hosts for VDDK transformation]
a|Go to:

* xref:Configuring_conversion_hosts_ansible_rhv_1-1_ssh[Configuring the RHV conversion hosts for SSH transformation]
...
* xref:Authenticating_conversion_hosts_in_cloudforms_rhv_1-1_vddk[Authenticating the RHV conversion hosts in CloudForms], in xref:Preparing_the_migration_environment_rhv_1-1_vddk[Preparing the migration environment]
|===
...
This guide describes how to plan your migration, to prepare the VMware and Red Hat OpenStack Platform (RHOSP) environments, and to migrate the virtual machines.
...
You can prepare the VMware source and Red Hat OpenStack Platform (RHOSP) environments.
...
You can install and configure Red Hat OpenStack Platform (RHOSP) and CloudForms.
...
You can create the Red Hat OpenStack Platform (RHOSP) conversion hosts, download VDDK, and configure the conversion hosts with Ansible playbooks.
...
You can configure the Red Hat OpenStack Platform (RHOSP) environment for SSH transformation if you cannot use VDDK.
...
The private key of the SSH key pair that is generated in this step will be used in xref:Configuring_conversion_hosts_ansible_osp_1-1_ssh[Configuring the RHOSP conversion hosts for SSH transformation].

|Instead of xref:Configuring_conversion_hosts_ansible_osp_1-1_vddk[Configuring the RHOSP conversion hosts for VDDK transformation]
a|Go to:

* xref:Configuring_conversion_hosts_ansible_osp_1-1_ssh[Configuring the RHOSP conversion hosts for SSH transformation]
* xref:Copying_vmware_ssh_keys_to_conversion_hosts_{context}[Copying the VMware SSH keys to the RHOSP conversion hosts]
|===
