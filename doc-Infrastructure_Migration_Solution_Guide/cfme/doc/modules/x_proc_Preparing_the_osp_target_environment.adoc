...
. Add the SSH key for the OpenStack Platform user (default is `cloud-user`) in CloudForms:
...
. Ensure that `cloud-user` has `admin` privileges for the target projects.
...
You are ready to xref:Configuring_osp_hosts[configure the conversion hosts].
