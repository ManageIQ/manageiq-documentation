...
To send SNMP traps from the CloudForms Management Engine server, you must have the `Notifier` server role and the SNMP daemons enabled. For information on enabling SNMP, see the link:https://access.redhat.com/documentation/en/red-hat-{product-title}/4.1/general-configuration/general-configuration[General Configuration]
guide.
