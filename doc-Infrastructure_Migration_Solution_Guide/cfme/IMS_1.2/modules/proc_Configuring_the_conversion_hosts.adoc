...
You can select, authenticate, and configure your conversion hosts for VDDK or SSH transformation. Configuring the conversion hosts involes the following tasks:

. VDDK only: xref:Downloading_vddk[Downloading VDDK]
. VDDK and SSH: xref:Configuring_conversion_hosts_in_cloudforms[Configuring the conversion hosts in CloudForms]
. SSH only: xref:Ssh_only_copying_the_vmware_keys_to_the_conversion_hosts[Copying the VMware keys to the conversion hosts]
. (Optional) xref:Verifying_conversion_hosts[Verifying the conversion hosts] using an API call in a browser. This procedure verifies the names and number of conversion hosts. It does not check their configuration.
...
* If you are using SSH transformation, there is an additional step, copying the VMware keys to the conversion hosts.

[id="Ssh_only_copying_the_vmware_keys_to_the_conversion_hosts"]
== SSH only: Copying the VMware keys to the conversion hosts

Copy the VMware keys to the conversion hosts:
