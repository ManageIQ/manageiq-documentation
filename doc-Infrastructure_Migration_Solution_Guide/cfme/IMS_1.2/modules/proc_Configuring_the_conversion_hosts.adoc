...
[IMPORTANT]
====
*Red Hat Virtualization*: If a RHV host has an existing SSH private key, you must delete the old key manually in `/var/lib/vdsm/.ssh/id_rsa` before configuring the conversion host.
====
...
. Click *Configure*.
|*Results*
|Wait for the conversion host configuration to finish and click *Close*.
|===
...
If you are using SSH transformation, you are ready to copy the VMware keys to the conversion hosts:
