...
.. Configuring the conversion host with the `conversion_host_enable` playbook
.. Verifying the configuration with the `conversion_host_check` playbook
.. SSH configuration: Copying the VMware keys to the conversion hosts
. (Optional) Verifying the name and number of conversion hosts in a browser
endif::osp[]
...
ifdef::rhv[]
. On each conversion host, run `ssh-keyscan` for each VMware hypervisor and output the public key to `known_hosts`, as in the following example:
+
[options="nowrap" subs="+quotes,verbatim"]
----
...
. Repeat the procedure on each conversion host, to ensure that all your conversion hosts have all the VMware keys.
endif::rhv[]
ifdef::osp[]
. On each conversion host, run `ssh-keyscan` for each VMware hypervisor and output the public key to `known_hosts`, as in the following example:
+
...
. Repeat the `ssh-keyscan` commands on each conversion host, to ensure that all your conversion hosts have all the VMware keys.
...
----
<1> You must perform these commands for all the VMware hypervisors.
...
== Authenticating the Red Hat Virtualization conversion hosts
