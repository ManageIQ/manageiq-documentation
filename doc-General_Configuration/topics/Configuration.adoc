...
[[adding-a-new-appliance-to-an-existing-region* Click *Hourly* to run the backup hourly. You will be prompted to select the number of hours between each analysis.
gion with a Non-default Password
...
Hourly database maintenance tasks such as reindexing is useful for highly active database tables with such as metrics, workers, and servers. 
...
. Enter `appliance_console`, and press *Enter*.
. Press any key.
. Select `10` to configure the automatic database maintenance schedule through a dialog.
...
To reset your database maintenance settings, enter *10) Configure Database Maintenance* again from the appliance console menu, and confirm that you want to unconfigure the settings in the configuration dialog to delete the previous settings. 

To configure a new database maintenance schedule, enter *10) Configure Database Maintenance* once again and configure the values using the dialog.
