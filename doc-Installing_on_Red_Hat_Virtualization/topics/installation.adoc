...
Uploading the {product-title} appliance file onto Red Hat Virtualization requires:
...
Refer to
https://access.redhat.com/documentation/en-us/red_hat_virtualization/4.1-beta/html-single/administration_guide/#sect-The_Image_Uploader_Tool[The Image Uploader Tool]
to for details on using the Image Uploader.
...
Upload the {product-title} appliance using the Image Uploader tool:
...
It takes approximately 90 minutes to upload the {product-title} appliance file to the Red Hat Enterprise Virtualization Manager. Once the `OVF` is uploaded and imported as a template, add a network adapter to the template itself.
