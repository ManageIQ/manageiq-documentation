...
|
								...
								Archived: Virtual Machine has no Host or Datastore associated with it. Archiving is done to move virtual machines to a low cost storage, either on demand or during retirement, if requested, to avoid incurring extra cost on a virtualized infrastructure due to Virtual Machine sprawl.
...
|
								...
								Orphaned: Virtual Machine has no Host but does have a Datastore associated with it. Orphaned virtual machines are those that have been removed from their providers but due to a connection error or process failure, still appear in the provider's storage, when CloudForms Management Engine runs a SmartState analysis on the provider.
