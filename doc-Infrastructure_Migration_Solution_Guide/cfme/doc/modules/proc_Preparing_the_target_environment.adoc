...
[NOTE]
====
Some of these hosts will be deployed as conversion hosts. The number of conversion hosts depends on your migration size and infrastructure capabilities. See xref:How_many_conversion_hosts[] for details.
====
...
You download the conversion host appliance and use it to deploy the conversion host instances. The number of conversion hosts you deploy depends on your migration size and infrastructure capabilities. See xref:How_many_conversion_hosts[] for details.
...
. Increase the disk storage space of the instance. The instance is created from an image, but the disk storage defined in the image is not large enough to accommodate the instance's file system. You can either extend the partition (extend the PV in the VG) to include all the required space or you can create a new partition and add it as a PV to the VG, and then increase the root LV and the filesystem:
+
[options="nowrap" subs="+quotes,verbatim"]
----
# TBD
