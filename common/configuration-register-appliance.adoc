...
... Enter the *Repository Name(s)*. The default is `cf-me-5.10-for-rhel-7-rpms rhel-7-server-ansible-2.6-rpms rhel-7-server-extras-rpms rhel-7-server-optional-rpms rhel-7-server-rpms`, which are the {product-title} repository and the Red Hat Software Collections repository.
...
... Enter the *Repository Name(s)*. The default is `cf-me-5.10-for-rhel-7-rpms rhel-7-server-ansible-2.6-rpms rhel-7-server-extras-rpms rhel-7-server-optional-rpms rhel-7-server-rpms`, which are the Red Hat CloudForms repository and the Red Hat Software Collections repository.
... To use a HTTP proxy, select *Use HTTP Proxy* and enter your proxy details.
