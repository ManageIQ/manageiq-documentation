...
* If you are migrating a large number of virtual machines or remigrating virtual machines, create a xref:CSV_file[CSV file] to import the virtual machines into the migration plan.
...
* If you are migrating RHEL virtual machines, set up the xref:Setting_up_an_ansible_environment[Ansible environment] and configure the xref:Rhel_premigration_playbook[RHEL pre-migration playbook].
...
. Perform the procedure for each migration plan screen:
