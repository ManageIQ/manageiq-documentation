...
* Running postmigration cleanup tasks, such as `fstrim`, to reduce the space required by virtual machines migrating to OpenStack Platform with Ceph storage (see xref:Installing_and_configuring_open_stack_platform[])
