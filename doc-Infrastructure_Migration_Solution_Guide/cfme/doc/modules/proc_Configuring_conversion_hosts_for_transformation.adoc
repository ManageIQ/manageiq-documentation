...
. Using SCP, copy the VDDK package to `/var/www/html` on the Manager machine (for Red Hat Virtualization) or to each conversion host (for OpenStack Platform)

You are ready to configure the conversion hosts.

.VDDK and SSH: Configuring the conversion hosts for transformation

Perform the following steps on the Manager machine (for Red Hat Virtualization) or on each conversion host (for OpenStack Platform):

. Create an Ansible inventory file, +/usr/share/ovirt-ansible-v2v-conversion-host-_version_/playbooks/conversion_hosts_inventory.yml+, according to the following example:
+
