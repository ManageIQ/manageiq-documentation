...
Red Hat CloudForms supports multi-tenancy, letting you isolate one tenantś resources and user accounts from another. In addition, tenants can be nested in a heirarchy, enabling those as the bottom to inherit permissions from those above. This configuration allows for granular user permissions to be set on specific tenants.
