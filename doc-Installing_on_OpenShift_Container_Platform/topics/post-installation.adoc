...
By default the route that is deployed as part of the template uses edge termination and the certificates that Openshift is installed with.  It is possible to change this in the Openshift UI by navigating to "Applications" -> "Routes", clicking on the route named "httpd", then selecting "Actions" -> "Edit".  Scroll down to the "Certificates" section, Here you can upload or paste the required certificate files, then click "Save".
