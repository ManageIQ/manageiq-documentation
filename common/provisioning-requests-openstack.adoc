Create a request to provision Red Hat OpenStack Platform cloud instances from images, image snapshots, volumes, and volume snapshots using {product-title}. Only bootable volumes not in use will be available. 
...
.. To change the source file to use as a basis for the instance, select it from the list images, snapshots, volumes, or volume snapshots.
