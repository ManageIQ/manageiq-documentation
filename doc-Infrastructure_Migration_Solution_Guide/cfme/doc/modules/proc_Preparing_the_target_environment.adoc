...
----
+
Otherwise, SSH will fail for the vdsm user (see link:https://bugzilla.redhat.com/show_bug.cgi?id=1544379[] for details). This issue cannot be resolved by modifying the configuration file because the file is restored during upgrades.
endif::rhv[]
ifdef::osp[]
