...
The [label]#Hosts# page under [label]#Infrastructure# displays the hosts discovered in your enterprise environment. 
...
CloudForms Management Engine allows you to compare hosts and check operating systems, host software and version information, and hardware. 

.Procedure: To Compare Hosts
. Navigate to menu:Infrastructure[Hosts]. 
. Check the hosts to compare. 
. Click  image:images/1847.png[] btn:[(Configuration)], and then  image:images/2148.png[] btn:[(Compare selected Hosts)].
  The comparison displays in a default expanded view, which lists a limited set of properties. 
. To remove a host from the comparison, click  image:images/1861.png[] btn:[(Remove this Host from the comparison)] at the bottom of the column. 
...
.Result:
CloudForms Management Engine allows you to fine tune your comparison by selecting categories to compare. 
