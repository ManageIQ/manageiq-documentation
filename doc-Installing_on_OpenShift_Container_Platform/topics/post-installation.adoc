...
To apply package updates to a {product-title_short} appliance running on OpenShift Container Platform, delete the pods and redeploy them using updated {product-title_short} container images. If you have modified the template, this requires additional steps in xref:modified-template-update[].
...
[IMPORTANT]
====
If you are updating a {product-title} 4.6.0 environment to version 4.6.1, follow the steps in xref:modified-template-update[]. When updating between these versions, the template must also be updated to reflect the updated ConfigMap mount path for `postgresql`. 
====
endif::cfme[]
...
[IMPORTANT]
====
If you are updating a {product-title} 5.9.0 environment to version 5.9.1, follow the steps in xref:modified-template-update[]. When updating between these versions, the template must also be updated to reflect the updated ConfigMap mount path for `postgresql`.
====
endif::miq[]
...
To deploy new container images for {product-title_short}, delete the pods, update the images, then rebuild the updated pods:
...
To update a {product-title_short} deployment with a modified template, update the container images and the template using the following steps:
...
ifdef::cfme[]
* If you are updating a {product-title} 4.6.0 environment to version 4.6.1, update the `postgresql` ConfigMap mount path from `postgresql-config` to `postgresql-cfg`.
endif::cfme[]

ifdef::miq[]
* If you are updating a {product-title} 5.9.0 environment to version 5.9.1, update the `postgresql` ConfigMap mount path from `postgresql-config` to `postgresql-cfg`.
endif::miq[]
====
+
----
