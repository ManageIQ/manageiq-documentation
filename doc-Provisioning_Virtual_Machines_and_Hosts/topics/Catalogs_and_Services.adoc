...
{product-title} supports adding AWS CloudFormation, OpenStack Heat, Microsoft Azure, VNF, and VMWare vApp template type, and provides the ability to:
...
[NOTE]
====
You can only edit the name and description of a read-only template as there can be stacks associated with the template. For templates that are not read-only, you can edit the template as needed.
====
+
. Click *Save*.
...
[NOTE]
====
To create a new template its content must be unique.
====
+
. Click *Save*.
