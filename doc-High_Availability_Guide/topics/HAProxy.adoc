...
The Virtual IP is the single IP address that is used as the point of access to the {product-title_short} web UI appliances and is configured in the HAProxy configuration along with a load balancer profile. When an end user accesses the Virtual IP, it directs traffic to the appropriate {product-title_short} appliance based on the configured HAProxy policy.
...
// INSERT edited architecture image
