...
. Configuring a VMware hypervisor to support additional connections if you are using VDDK and performing more than ten concurrent migrations from that hypervisor. See xref:Configuring_vmware_hypervisor_for_more_than_10_concurrent_vddk_migrations_{context}[].
