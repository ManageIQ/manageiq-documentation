...
* From the *User Type* list, select one of the following: [QUESTION: When do you use each user type, and how do you configure the values for them?]
** Select *User Principal Name* to type the user name in the format of user@domainname. 
** Select *Email Address* to login with the user's email address.
** Select *Distinguished Name* (CN=<user>) or *Distinguished Name* (UID=<user>) to use just the user name, but be sure to enter the proper *User Suffix* for either one [QUESTION: What is the proper suffix?]. Choose the correct *Distinguished Name* option for your directory service implementation.
...
* Check *Get User Groups from LDAP* to [QUESTION: What exactly does this set?]. 
+
[IMPORTANT]
======
...
(NOTE:need more details on what to configure)
. Click *Save*.
...
* Type in the *cn* for the group in *LDAP Group*. This group must be in the LDAP directory source you specified under Operations-Server [QUESTION: Which step was this specified in? How is "Adding A Group" configured, as there is no *LDAP Group* field?].
+
image:2090.png[]
+
...
. Change groups by clicking on the user pull down on the top right of the screen. There should be a dropdown list of the groups the user is authorized for. 
