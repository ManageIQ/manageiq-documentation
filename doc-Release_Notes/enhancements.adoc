...
CloudForms Management Engine can now report a high watermark for the maximum number of virtual machines on a provider over a one-month period, which can be used for billing. Reports can be queued to collect watermark information for host sockets per provider and for virtual machines per provider. To access the *Watermark Host Sockets per Provider* and *Watermark VMs per Provider* reports, navigate to menu:Cloud Intelligence[Reports > Configuration Management > Providers].
+
