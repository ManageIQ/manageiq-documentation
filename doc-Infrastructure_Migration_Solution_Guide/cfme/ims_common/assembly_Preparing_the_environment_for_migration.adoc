[id="Preparing_the_environment_for_migration"]
= Preparing the environment for migration

Preparing your environment for migration involves the following key steps:

* xref:Preparing_the_vmware_source_environment[Preparing the VMware source environment]:

. Extending the VMware network to the target environment
. xref:Preparing_the_source_virtual_machines[]
. xref:Configuring_the_vmware_hypervisors_for_ssh_transformation[], if necessary

* xref:Preparing_the_target_environment[Preparing the target environment]:

** xref:Preparing_the_rhv_target_environment[]
** xref:Preparing_the_osp_target_environment[]

* xref:Configuring_the_conversion_hosts[Configuring the conversion hosts]

include::proc_Preparing_the_vmware_source_environment.adoc[leveloffset=+1]

[id="Preparing_the_target_environment"]
= Preparing the target environment

Prepare your target environment for migration:

* xref:Preparing_the_rhv_target_environment[]
* xref:Preparing_the_osp_target_environment[]

[id="Preparing_the_rhv_target_environment"]
== Preparing the Red Hat Virtualization environment

Preparing the Red Hat Virtualization environment involves the following key steps:

. xref:Installing_and_configuring_red_hat_virtualization[Installing and configuring Red Hat Virtualization]
. xref:Cloudforms_for_rhv[Installing and configuring CloudForms]
. xref:Reinstalling_ipa_client[Reinstalling `ipa-client`], if you are configuring the conversion hosts to use SSSD with single sign-on

:context: rhv
:rhv:
include::ref_Target_prerequisites.adoc[leveloffset=+1]
include::../modules/proc_Preparing_the_target_environment.adoc[leveloffset=+2]
:rhv!:

[id="Preparing_the_osp_target_environment"]
== Preparing the Red Hat OpenStack Platform environment

Preparing the Red Hat OpenStack Platform environment involves the following key steps:

. xref:Installing_and_configuring_open_stack_platform[Installing and configuring Red Hat OpenStack Platform]
. xref:Cloudforms_for_osp[Installing and configuring CloudForms]
. xref:Deploying_osp_conversion_hosts[Deploying the Red Hat OpenStack Platform conversion hosts]

:context: osp
:osp:
include::ref_Target_prerequisites.adoc[leveloffset=+1]
include::../modules/proc_Preparing_the_target_environment.adoc[leveloffset=+2]
:osp!:

include::../modules/proc_Configuring_the_conversion_hosts.adoc[leveloffset=+1]
