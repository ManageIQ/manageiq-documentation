...
* If you are migrating a large number of virtual machines or re-migrating virtual machines, create a xref:Creating_a_csv_file_to_add_virtual_machines_to_the_migration_plan[CSV file] to add the virtual machines to the migration plan.
...
* If you are migrating RHEL or other Linux virtual machines, set up the xref:Setting_up_an_ansible_environment[Ansible environment] and configure the xref:Configuring_the_rhel_premigration_playbook[RHEL pre-migration playbook].
