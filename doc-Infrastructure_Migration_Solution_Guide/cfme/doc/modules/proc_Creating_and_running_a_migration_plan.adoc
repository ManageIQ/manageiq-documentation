...
You create and run a migration plan with the *Create Migration Plan* wizard.

In the *Schedule* screen of the wizard, you can select one of the following options:

* *Save migration plan to run later*
* *Start migration immediately*

To cancel a migration plan in progress, see xref:Canceling_a_migration_plan[].

If a migration fails because of external factors, such as network disruption, see xref:Retrying_a_failed_migration_plan[].

== Creating a migration plan[[Creating_a_migration_plan]]

. Click menu:Compute[Migration > Migration Plans].
. Click *Create Migration Plan*.
. Perform the following procedures in the *Create Migration Plan* wizard:

.Create Migration Plan wizard
